    ...
    def forward(self, preds, batch):
        ...
        loss_dict = dict(
            resi_gaussians=dict(
                ...
                mse_covs=F.mse_loss(predicted_covars, true_covars).detach() if loss_weights["resi_gaussians"]["mse_covs"] is not None else None,
                kldiv=metrics.kl_divergence_mvn(true_means, predicted_covars, true_means, true_covars).detach() if loss_weights["resi_gaussians"]["kldiv"] is not None else None,
                ...
                frob_norm=(metrics.frobenius_norm(predicted_covars - true_covars)/metrics.frobenius_norm(true_covars)).detach()  if loss_weights["resi_gaussians"]["frob_norm"] is not None else None,
                ...
                affine_invariant_dist = metrics.affine_invariant_distance(predicted_covars, true_covars).detach()  if loss_weights["resi_gaussians"]["affine_invariant_dist"] is not None else None,
            ...
            resi_correlations=dict(
                # mse=F.mse_loss(predicted_corrs, true_corrs, reduction="sum")/num_entries if loss_weights["resi_correlations"]["mse"] is not None else None,
